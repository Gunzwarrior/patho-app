...
def test_ordinary_pending_case_deletion_removes_case(mutable_db):
    _save("DELETE-PENDING")

    assert database.delete_pending_case("DELETE-PENDING")
    assert database.get_case_by_number("DELETE-PENDING") is None
    assert not database.delete_pending_case("DELETE-PENDING")


def test_validated_case_cannot_be_deleted_directly(mutable_db):
    _save("DELETE-VALIDATED", status="validated")

    assert not database.delete_pending_case("DELETE-VALIDATED")
    assert database.get_case_by_number("DELETE-VALIDATED")["status"] == "validated"


def test_returned_to_pending_case_deletion_cleans_owned_history_and_references(mutable_db):
    _save("DELETE-RETURNED", status="validated")
    assert database.return_case_to_pending("DELETE-RETURNED", "entered in error")
    case = database.get_case_by_number("DELETE-RETURNED")
    ...
    assert database.delete_pending_case("DELETE-RETURNED")
    conn = database.get_db_connection()
    try:
        for table in (
...
def test_deletion_rolls_back_when_case_delete_fails(mutable_db):
    _save("DELETE-ROLLBACK", status="validated")
    assert database.return_case_to_pending("DELETE-ROLLBACK", "correction")
    case = database.get_case_by_number("DELETE-ROLLBACK")
    conn = database.get_db_connection()
    try:
        before = {
    ...
    assert not database.delete_pending_case("DELETE-ROLLBACK")
    assert database.get_case_by_number("DELETE-ROLLBACK")["status"] == "pending"
    conn = database.get_db_connection()
    try:
...
def test_batch_provenance_is_removed_only_when_its_last_case_is_deleted(mutable_db):
    _save("DELETE-BATCH-ONE")
    _save("DELETE-BATCH-TWO")
    conn = database.get_db_connection()
    try:
        revision_id = conn.execute("SELECT id FROM Content_Revisions ORDER BY id DESC LIMIT 1").fetchone()[0]
        ...
        conn.execute(
            ...
            (batch_id, "DELETE-BATCH-ONE", "DELETE-BATCH-TWO"),
        )
        conn.commit()
    finally:
        conn.close()

    assert database.delete_pending_case("DELETE-BATCH-ONE")
    conn = database.get_db_connection()
    try:
        assert conn.execute("SELECT 1 FROM Case_Batch_Imports WHERE id = ?", (batch_id,)).fetchone()
    finally:
        conn.close()
    assert database.delete_pending_case("DELETE-BATCH-TWO")
    conn = database.get_db_connection()
    try:
